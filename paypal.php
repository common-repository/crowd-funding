 ...
 */
class CF_PayPal{
	// TODO change all this to production settings
	...
	const APPLICATION_ID = 'APP-80W284485P519543T';
	...
	function get_auth_url($project, $reward, $funder){
		...
		$preapproval->cancelUrl = add_query_arg(array('cf' => 'cancel_funding', 'funder_id' => $funder->ID), get_site_url());
		$preapproval->returnUrl = add_query_arg(array('cf' => 'funded', 'funder_id' => $funder->ID), get_site_url());
		$preapproval->ipnNotificationUrl = add_query_arg(array(
			'cf' => 'paypal_ipn',
			'funder_id' => $funder->ID,
		), get_site_url());
		
		$preapproval->memo = sprintf(__("Funding '%s' from %s for the reward '%s' if we reach our target.", 'crowdfunding'), $project->post_title, get_bloginfo('name'), $reward->post_title);
		
		// The client details
		$preapproval->clientDetails = new ClientDetailsType();
		$preapproval->clientDetails->applicationId = self::APPLICATION_ID;
		...
		if(strtoupper($adaptivePayment->isSuccess) == 'FAILURE'){
			...
			return self::PAYPAL_REDIRECT_URL.'_ap-preapproval&preapprovalkey='.$response->preapprovalKey;
		}
	}
	...
	function charge_funder($funder){
		...
		$payRequest->clientDetails->applicationId = self::APPLICATION_ID;
		...
		$receiver->email = isset($cf_paypal['email']) ? $cf_paypal['email'] : $admin_info->user_email;
		$receiver->amount = get_post_meta($funder->ID, 'funding_amount', true);
		$receivers[] = $receiver;
	...
	private function verify_ipn($vars = null){
		...
		$fp = fsockopen ('ssl://'.CF_PayPal::PAYPAL_HOST, 443, $errno, $errstr, 30);
