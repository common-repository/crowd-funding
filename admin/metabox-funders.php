<div id="project-funders">
	<?php foreach($funders as $funder) : ?>
		...
		<div class="funder" data-charged="<?php !empty($charged) ? 'true' : 'false' ?>" data-funder-id="<?php print $funder->ID ?>" data-project-id="<?php print $project->ID ?>">
			<div class="avatar">
				<a href="mailto:<?php print $funder_info['email'] ?>" title="<?php printf(__('Email %s', 'crowdfunding'), $funder_info['name']) ?>">
	...
	<p>
		...
		<a href="<?php print wp_nonce_url(add_query_arg(array('cfa' => 'export_funders', 'project' => $post->ID), site_url()), 'export_funders') ?>" target="_blank">Download</a>
		<p class="description"><?php _e("You can collect funding after you've reached your target") ?></p>
	</p>
</div>