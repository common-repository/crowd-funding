jQuery(function($){
	...
	$( "input[name=cf_target_date]" ).datepicker();
	...
	$('#collect-funding').click(function(){
		if(confirm('Are you sure you want to collect funding?')){
			...
			var displayDonationModal = function(){
				$('#cf-modal-donate, #cf-modal-donate-overlay').fadeIn();
				$('#cf-modal-donate input[name=currency_code]').val(crowdFunding.currency);
				
				$('#cf-modal-donate .close').click(function(){
					$('#cf-modal-donate, #cf-modal-donate-overlay').fadeOut();
					return false;
				});
				
				$('#cf-modal-donate .paypal').click(function(){
					$('#donate-form').submit();
				});
			}
	...
	// Hide the admin message
	$('#cf-admin-message a.close').click(function(){
		var $$ = $(this);
		$.get($$.attr('href'));
		$('#cf-admin-message').slideUp('normal', function(){$(this).remove();});
		return false;
	});
});